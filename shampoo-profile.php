<!DOCTYPE html>
<html lang="en">
    <head>
        <title>Shampoo | Pure Fusion</title>
        <meta charset="utf-8">
        <link rel="stylesheet" href="/skincare/styles.css">
        <link rel="stylesheet" href="/skincare/product-styles.css">
        <link rel="shortcut icon" type="image/png" href="/skincare/img/favicon.png"/>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <script src="animation.js" async></script>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    </head>
    <?php
    include('navigation-bar1.php');
    ?>
    ?>
    <body>
        <div class="container">
            <h2>SHAMPOO</h2>
        </div>
        <div class="grid-container">
            <div class="product">
                <img class="pic" src="/skincare/img/shampoo1.jpeg" alt="Shampoo 1">
                <div class="prod-info">
                    <p class="title">Mango (50ml)</p>
                    <p class="price">$60</p>
                    <button class="img-btn">Add to Cart</button>
                </div>
            </div>
            <div class="product">
                <img class="pic" src="/skincare/img/shampoo2.jpeg" alt="Shampoo 2">
                <div class="prod-info">
                    <p class="title">Shea (50ml)</p>
                    <p class="price">$60</p>
                    <button class="img-btn">Add to Cart</button>
                </div>
            </div>
            <div class="product">
                <img class="pic" src="/skincare/img/shampoo3.jpeg" alt="Shampoo 3">
                <div class="prod-info">
                    <p class="title">Rich Plum (50ml)</p>
                    <p class="price">$60</p>
                    <button class="img-btn">Add to Cart</button>
                </div>
            </div>
            <div class="product">
                <img class="pic" src="/skincare/img/shampoo4.jpeg" alt="Shampoo 4">
                <div class="prod-info">
                    <p class="title">Fuji Green Tea (50ml)</p>
                    <p class="price">$60</p>
                    <button class="img-btn">Add to Cart</button>
                </div>
            </div>
        </div>
    </body>
    <?php
     include('footer.php');
    ?>
</html>